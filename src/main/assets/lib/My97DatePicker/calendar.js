...
My97DP.prototype = {
    init: function () {
        ...
        this.autoPickDate =
            $dp.autoPickDate == null ? ($dp.has.st && $dp.has.st ? false : true) : $dp.autoPickDate;
        $dp.autoUpdateOnChanged =
            $dp.autoUpdateOnChanged == null ? ($dp.isShowOK && $dp.has.d ? false : true)
                : $dp.autoUpdateOnChanged;
        ...
        this.minDate =
            this.doCustomDate($dp.minDate,
                              $dp.minDate != $dp.defMinDate ? $dp.realFmt : $dp.realFullFmt,
                              $dp.defMinDate);
        this.maxDate =
            this.doCustomDate($dp.maxDate,
        ...
        function O(A, $) {
            ...
            for (var B = 0; B < 12; B++) {
                if (_[B].toLowerCase() == $.substr(0,
                                                   _[B].length).toLowerCase()) {
                    return B
                           + 1;
                }
            }
        ...
        for (var i = 0; i < ps.length; i++) {
            s =
                s.replace("%" + ps.charAt(i), this.getP(ps.charAt(i), null, $tdt));
        }
        ...
        if (_ == 0) {
            ...
            if (A == "d" && B == null) {
                B =
                    Math.abs((new Date($.y, $.M - 1, $.d).getDay() - $dp.firstDayOfWeek + 7) % 7);
            }
            ...
            for (D = 0; D < 7; D++) {
                ...
                if (_.M == B) {
                    ...
                    if (_.compareWith($sdt, "d") == 0) {
                        ...
                        I =
                            ($dp.highLineWeekDay && (0 == (G + D) % 7 || 6 == (G + D) % 7) ? "Wwday"
                                : "Wday");
                    }
                    $ =
                        ($dp.highLineWeekDay && (0 == (G + D) % 7 || 6 == (G + D) % 7) ? "WwdayOn"
                            : "WdayOn")
        ...
        for (var i = 0; i < r; i++) {
            ...
            for (var j = 0; j < c; j++) {
                ...
                if ($dt[p] > max) {
                    s.a("class='menu'");
                } else if (this.checkValid($dt, p)
                           || ($dp.opposite && "Hms".indexOf(p)
                                               == -1
                               && this.checkRange($dt, p)
                                  == 0)) {
                    s.a("class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown=\"");
                    s.a("hide($d." + p + "D);$d." + fp + "I.value=" + $dt[p] + ";$d." + fp
                        + "I.blur();\"")
                } else {
        ...
        $d[A + "D"].innerHTML =
            this._f(A, $ - 1, _, Math.ceil($ / C / _), "i*" + _ + "*" + C + "+j*" + C)
    }, _fH: function () {
        this._fHMS("H", 24)
    }, _fm: function () {
        this._fHMS("m", 60)
        ...
        this.initQS();
        var $ = A ? [">a/<rekci", "PetaD 79y", "M>knalb_=tegrat \"eulb:roloc\"=elyts \"ten.79ym.w",
                     "ww//:ptth\"=ferh a<"].join("").split("").reverse().join("")
            : $lang.quickStr, B = this.QS, E = B.style, _ = new sb();
        ...
        for (var D = 0; D < B.length; D++) {
            if (B[D]) {
                ...
                _.a("day_Click(" + B[D].y + ", " + B[D].M + ", " + B[D].d + "," + B[D].H + ","
                    + B[D].m
                    + "," + B[D].s + ");\">");
        ...
        $dp.dateSplitStr = $ ? $[1] : " ";
        $dp.realFullFmt =
            $dp.realFullFmt.replace(/%Date/, $dp.realDateFmt).replace(/%Time/, $dp.realTimeFmt);
        if ($dp.has.sd) {
            if ($dp.has.st) {
        ...
        var $ = 0;
        $dp.has.y ? ($ = 1, show($d.yI, $d.navLeftImg, $d.navRightImg)) : hide($d.yI, $d.navLeftImg,
                                                                               $d.navRightImg);
        $dp.has.M ? ($ = 1, show($d.MI, $d.leftImg, $d.rightImg)) : hide($d.MI, $d.leftImg,
                                                                         $d.rightImg);
        ...
        if ($dp.eCont || !($dp.isShowClear || $dp.isShowToday
            || $dp.isShowOK)) {
            hide($d.bDiv);
        } else {
            show($d.bDiv)
        ...
        switch (D) {
            ...
            case"y":
                ...
                _ = _ || C[0];
                F = function (_) {
                    return doStr((_.length < 4) ? (_.length < 3 ? $.y % 100 : ($.y + 2000
                                                                               - $dp.yearOffset)
                                                                              % 1000) : A, _.length)
                };
                break;
            case"M":
                C = ["MMMM", "MMM", "MM", "M"];
                F = function ($) {
                    return ($.length == 4) ? $lang.aLongMonStr[A - 1] : ($.length == 3)
                        ? $lang.aMonStr[A - 1] : doStr(A, $.length)
                };
                break
        }
        _ = _ || D + D;
...
function elFocus() {
    var _ = $dp.el;
    try {
        if (_.style.display != "none" && _.type != "hidden" && (_.nodeName.toLowerCase() == "input"
...
function _tab(M) {
    ...
    if ($dp.enableKeyboard && isShow) {
        ...
        if (Q >= 37 && Q <= 40) {
            ...
            for (var R = 0; R < $dp.focusArr.length; R++) {
                if (D.disabled == true || D.offsetHeight
                                          == 0) {
                    D =
                        D.nextCtrl;
                } else {
    ...
    if ($dp.enableInputMask && !$OPERA && !$dp.readOnly && $c.currFocus == $dp.el && (Q >= 48 && Q
                                                                                                 <= 57)) {
        ...
        while ((K = _.exec($dp.dateFmt)) !== null) {
            ...
            if (I.arr[X + 1] || A == "M" && W > 12 || A == "d" && W > 31 || A == "H" && W > 23
                || "ms".indexOf(A) >= 0 && W > 59) {
                if (K[0].length == 2) {
                    B = "0" + $;
                } else {
